...
<p>For almost 50 years, she has defended her home against extractive industries like mining and logging, and more recently, {g('organized crime', 'Criminal enterprises that capitalize on political, social, and economic pressures for financial gain')}. Hundreds of environmental defenders in the region have been <strong>killed for the same work</strong>.</p>
</div>
""", unsafe_allow_html=True)
else:
    st.markdown(f"""
...
<p>This suggests the threat organized crime poses to people defending the environment around the world. However, these results have to be put into perspective, given limitations regarding attack records and the small scope of the analysis.</p>
...
<p>Things may be starting to change. As Kristine Perry says, countries are building "better protection mechanisms for defenders." While enforcement remains the missing link, international pressure is mounting. The upcoming UN Climate Change Conference will serve as a platform where organizations are set to demand the creation of an environmental justice group to safeguard the lives of those who protect the planet.</p>
